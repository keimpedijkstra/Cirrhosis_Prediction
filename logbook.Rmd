...
data["Stage"][data["Stage"] == 1] <- "one"
data["Stage"][data["Stage"] == 2] <- "two"
data["Stage"][data["Stage"] == 3] <- "three"
data["Stage"][data["Stage"] == 4] <- "four"
...
write.table(data_no_na, "C:/Users/keimp/Cirrhosis_Prediction/data/data_eda_no_na.csv", sep = ",", na = "", row.names = F)
...
ggplot(data=comparison_numeric, aes(x=rownames(comparison_numeric), y=percent_correct, fill=rownames(comparison_numeric))) +
  geom_bar(stat="identity") + theme(axis.text.x = element_blank()) + scale_fill_manual(values=c(hue_pal()(20))) + geom_hline(yintercept = comparison_numeric[1,1], linetype = "dashed")
```
